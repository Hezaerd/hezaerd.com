...
Clyde is the Operator business co-pilot. Portal already hosts Operator and Client
shells. Eve needs its own `agent/` root, deploy, and channel secrets.

## Decision

Clyde is `apps/clyde` (Eve), a workspace sibling of Portal — not embedded in
`apps/portal`. Tools will call Operator-scoped backend APIs; Clyde does not
...
Clyde is the co-pilot process beside it.
